...
import { X, Github, Sparkles, ShieldCheck, AlertCircle, Key } from 'lucide-react';
...
export function OAuthModal({ isOpen, onClose, onLoginSuccess, referralCode }: OAuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden p-6 sm:p-8">
        ...
        {/* Invited via Referral Banner */}
        {referralCode && (
          <div className="mb-5 p-3.5 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl text-indigo-200 text-xs flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-2.5">
              <Key className="w-4 h-4 text-indigo-400 shrink-0" />
              <div>
                <span className="font-bold text-white">Invited via Referral Token</span>
                <div className="text-[11px] text-indigo-300 font-mono">Code: {referralCode}</div>
              </div>
            </div>
        ...
        <div className="space-y-3.5">
          ...
          <a
            href={`/api/auth/github/login${referralCode ? `?ref=${referralCode}` : ''}`}
          ...
          <button
            ...
            className="w-full p-3.5 bg-slate-800/90 hover:bg-slate-800 border border-slate-700/80 text-slate-200 font-medium rounded-2xl shadow-sm hover:border-slate-600 hover:scale-[1.01] transition-all duration-200 flex items-center justify-between cursor-pointer"
          >
            <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={isLoading !== null}
            className="w-full p-3 bg-slate-900/60 hover:bg-slate-800/50 border border-slate-800 text-slate-400 font-medium rounded-2xl text-xs flex items-center justify-between transition-all cursor-pointer"
          >
            <div className="flex items-center gap-2.5">
